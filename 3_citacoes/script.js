const quotes = [
  ...
  const quotesTexto = document.querySelector('.text');
  const quotesAutor = document.querySelector('.author');

  function getQuote(){
    const index = Math.floor(Math.random() * quotes.length);
    
    quotesTexto.textContent = quotes[index].quote;
    quotesAutor.textContent = quotes[index].author;
